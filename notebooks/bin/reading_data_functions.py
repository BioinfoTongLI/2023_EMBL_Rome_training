...
def collect_spots_from_mat_files(
    extracted_spots_path, C, R, tiles_to_load, tile_names, tile_size
):
    ...
    for y_ind in range(
        tiles_to_load["y_start"], tiles_to_load["y_end"] + 1
    ):  # range(3,4):#
        for x_ind in range(
            tiles_to_load["x_start"], tiles_to_load["x_end"] + 1
        ):  # range(2,3):#
            ...
            if np.isin(tile_name, tile_names["selected_tile_names"]):
                ...
                extracted_spots = loadmat(
                    extracted_spots_path + "tile_" + tile_name + ".mat", squeeze_me=True
                )
                try:
                ...
                if N_i > 0:
                    ...
                    X = (
                        x_ind - tiles_to_load["x_start"]
                    ) * tile_size + extracted_spots["centers"][:, 0]
                    Y = (
                        y_ind - tiles_to_load["y_start"]
                    ) * tile_size + extracted_spots["centers"][:, 1]
